...
void makeJetP4() { 
	...
	/* if(DEBUG) {
        ...
        gSystem->Exec(Form("./example/fastjet_example < OneEvent_PFCands_tmp_%i.dat > OneEvent_PFJets_tmp_%i.dat", ib, ib));
        ...
        if(fin.is_open()) { 
            while(fin.good()){
                ...
                if(TMath::Sqrt(px*px+py*py)>2.95) {
                    ...
                    if(DEBUG) {
		      if(TMath::Sqrt(px*px+py*py)>2.98 && TMath::Sqrt(px*px+py*py)<3.02){
		      cout << event_ << " " 
                             << id << " " 
        ...
        // remove text files
        //if(!DEBUG ) 
	gSystem->Exec("'rm' OneEvent_PFCands_tmp*.dat OneEvent_PFJets_tmp*.dat");
